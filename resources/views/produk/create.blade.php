    ...
    <div class="bg-white border-b border-slate-200">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-2xl font-semibold text-slate-800">Tambah Produk</h1>
                    <p class="text-sm text-slate-500 mt-1">Lengkapi form untuk menambah produk baru</p>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            <form action="{{ route('produk.store') }}" method="POST" class="p-8">
                ...
                <div class="mb-8">
                    ...
                    <select name="status_id" 
                        ...
                        @foreach($status as $stat)
                            <option value="{{ $stat->id_status }}" {{ old('status_id') == $stat->id_status ? 'selected' : '' }}>
                                {{ $stat->nama_status }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="flex items-center gap-3 pt-6 border-t border-slate-200">
                    <button type="submit" 
                            class="flex-1 inline-flex items-center justify-center gap-2 bg-slate-800 hover:bg-slate-900 text-white px-6 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 shadow-sm hover:shadow-md">
